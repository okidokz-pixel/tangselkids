...
export function LearningCenterForm({ initial, id, initialDraftId }: { initial?: Record<string, any>; id?: string; initialDraftId?: string }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeTab, setActiveTab] = useState("general");
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [saveError, setSaveError] = useState("");
  const draft = useDraft("learning-centers", initialDraftId);
  ...
  // Course info
  const [courseTypes, setCourseTypes] = useState<string[]>(initial?.course_types ?? []);
  ...
  const [teacherStudentRatio, setTeacherStudentRatio] = useState(initial?.teacher_student_ratio ?? "");
  const [freeTrial, setFreeTrial] = useState<boolean | "">(
    initial?.free_trial === true ? true : initial?.free_trial === false ? false : ""
  );

  // Fees
  const [registrationFeeMin, setRegistrationFeeMin] = useState(initial?.registration_fee_min ?? "");
  const [registrationFeeMax, setRegistrationFeeMax] = useState(initial?.registration_fee_max ?? "");
  ...
  function toggleCourseType(ct: string) {
    setCourseTypes((prev) => prev.includes(ct) ? prev.filter((x) => x !== ct) : [...prev, ct]);
  }
  ...
  function buildPayload() {
    ...
    return {
      ...
      teacher_student_ratio: teacherStudentRatio || null,
      free_trial: freeTrial === "" ? null : freeTrial,
      registration_fee_min: registrationFeeMin ? Number(registrationFeeMin) : null,
      registration_fee_max: registrationFeeMax ? Number(registrationFeeMax) : null,
  ...
  async function handleSave() {
    ...
    startTransition(async () => {
      try {
        await saveLearningCenter(id ?? null, buildPayload(), draft.draftId);
      } catch (e: unknown) {
  ...
  const TABS = ["general", "course", "fees", "media"];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => router.push("/admin/learning-centers")}
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>
            {id ? name || "Edit Tempat Kursus" : "Tambah Tempat Kursus"}
          </h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FillDataButton category="learning-centers" name={name} address={address} googlePlaceId={googlePlaceId} instagram={instagram} website={website} onResult={applyFill} disabled={isPending} />
      ...
      {activeTab === "general" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Location Detail">
            <input style={inputStyle} value={locationDetail} onChange={(e) => setLocationDetail(e.target.value)} placeholder="e.g. Sektor 7, Bintaro" />
          </Field>
          ...
          <Field label="Jam Buka">
            <input style={inputStyle} value={hours} onChange={(e) => setHours(e.target.value)} placeholder="e.g. Senin–Sabtu 09:00–18:00" />
          </Field>
      ...
      {activeTab === "course" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Rentang Usia">
            <input style={inputStyle} value={ageRange} onChange={(e) => setAgeRange(e.target.value)} placeholder="e.g. 4–12 tahun, 3 tahun ke atas" />
          </Field>

          <Field label="Bahasa Pengantar">
            <select style={inputStyle} value={teachingLanguage} onChange={(e) => setTeachingLanguage(e.target.value)}>
              <option value="">— Select —</option>
              {TEACHING_LANG_OPTIONS.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>
          </Field>

          <Field label="Rasio Guru:Murid">
            <input style={inputStyle} value={teacherStudentRatio} onChange={(e) => setTeacherStudentRatio(e.target.value)} placeholder="e.g. 1:6, Mandiri (self-paced)" />
          </Field>

          <Field label="Free Trial">
            <select style={inputStyle} value={freeTrial === "" ? "" : freeTrial ? "true" : "false"}
              onChange={(e) => setFreeTrial(e.target.value === "" ? "" : e.target.value === "true")}>
      ...
      {activeTab === "fees" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          <SectionDivider label="Biaya Pendaftaran (Registration Fee)" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Min (Rp)"><input style={inputStyle} type="number" value={registrationFeeMin} onChange={(e) => setRegistrationFeeMin(e.target.value)} /></Field>
            <Field label="Max (Rp)"><input style={inputStyle} type="number" value={registrationFeeMax} onChange={(e) => setRegistrationFeeMax(e.target.value)} /></Field>
          </div>

          <SectionDivider label="Biaya Bulanan (Monthly Fee)" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ display: "grid", gap: 28 }}>
          <div>
            ...
            <ImageUpload
              ...
              bucket="school-logo"
              path={`lc-${id ?? "new"}/logo_${Date.now()}.jpg`}
          ...
          <div>
            ...
            <PhotoGrid
              ...
              bucket="learning-center-image"
              entityId={id ?? "new"} maxPhotos={10}
            />
          </div>

          <div>
            <SectionDivider label="Fee Detail Image" />
            <ImageUpload
              value={feeImageUrl} onChange={setFeeImageUrl}
              bucket="price-detail"
              path={`lc-${id ?? "new"}/fee_${Date.now()}.jpg`}
              label="" aspectRatio="16/9" width={320} height={180}
            />
          </div>
