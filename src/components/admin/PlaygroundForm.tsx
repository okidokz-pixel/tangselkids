...
import { savePlayground, deletePlayground } from "@/app/admin/actions";
...
export function PlaygroundForm({ initial, id, initialDraftId }: { initial?: Record<string, any>; id?: string; initialDraftId?: string }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeTab, setActiveTab] = useState("general");
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [saveError, setSaveError] = useState("");
  const draft = useDraft("playgrounds", initialDraftId);
  ...
  // Playground info
  const [playgroundType, setPlaygroundType] = useState(initial?.playground_type ?? "");
  const [playgroundTypeRaw, setPlaygroundTypeRaw] = useState(initial?.playground_type_raw ?? "");
  ...
  function buildPayload() {
    ...
    return {
      ...
      playground_type: playgroundType || null,
      playground_type_raw: playgroundTypeRaw || null,
  ...
  function applyFill(d: PlaceFillData) {
    ...
    if (d.facilities.length && !facilities) setFacilities(d.facilities.join(", "));
  }
  ...
  async function handleSave() {
    ...
    startTransition(async () => {
      try {
        await savePlayground(id ?? null, buildPayload(), draft.draftId);
      } catch (e: unknown) {
  ...
  const TABS = ["general", "info", "fees", "media"];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => router.push("/admin/playgrounds")}
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>
            {id ? name || "Edit Playground" : "Tambah Playground"}
          </h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FillDataButton category="playgrounds" name={name} address={address} googlePlaceId={googlePlaceId} instagram={instagram} website={website} onResult={applyFill} disabled={isPending} />
      ...
      {activeTab === "general" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Location Detail">
            <input style={inputStyle} value={locationDetail} onChange={(e) => setLocationDetail(e.target.value)} placeholder="e.g. Mall Alam Sutera Lt. 3" />
          </Field>
          ...
          <Field label="Jam Buka">
            <input style={inputStyle} value={hours} onChange={(e) => setHours(e.target.value)} placeholder="e.g. Setiap hari 10:00–22:00" />
          </Field>

          <Field label="About (ID)">
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
              <ImproveButton text={about} name={name} category="playground" onResult={setAbout} />
              <TranslateButton
      ...
      {activeTab === "info" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          <Field label="Tipe (normalized — for filter)">
            <select style={inputStyle} value={playgroundType} onChange={(e) => setPlaygroundType(e.target.value)}>
          ...
          <Field label="Tipe Raw (displayed to users)">
            <input style={inputStyle} value={playgroundTypeRaw} onChange={(e) => setPlaygroundTypeRaw(e.target.value)} placeholder="e.g. Indoor, Trampoline" />
            <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>Free text — shown as-is on the place page and card.</div>
          </Field>

          <Field label="Fasilitas (ID)">
            <textarea style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} value={facilities} onChange={(e) => setFacilities(e.target.value)} placeholder="List facilities, one per line or comma-separated" />
          </Field>

          <Field label="Fasilitas (EN)">
            <textarea style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} value={facilitiesEn} onChange={(e) => setFacilitiesEn(e.target.value)} />
          </Field>
        </div>
      )}
      ...
      {activeTab === "fees" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          <SectionDivider label="Harga Tiket" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ display: "grid", gap: 28 }}>
          <div>
            ...
            <ImageUpload
              ...
              bucket="playground-logo"
              path={`pg-${id ?? "new"}/logo_${Date.now()}.jpg`}
          ...
          <div>
            ...
            <PhotoGrid
              ...
              bucket="playground-photos"
              entityId={id ?? "new"} maxPhotos={10}
            />
          </div>

          <div>
            <SectionDivider label="Detail Image (shown free on place page)" />
            <ImageUpload
              value={feeImageUrl} onChange={setFeeImageUrl}
              bucket="playground-detail"
              path={`pg-${id ?? "new"}/detail_${Date.now()}.jpg`}
