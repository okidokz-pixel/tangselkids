...
import { saveCafe, deleteCafe } from "@/app/admin/actions";
...
export function CafeForm({ initial, id, initialDraftId }: { initial?: Record<string, any>; id?: string; initialDraftId?: string }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeTab, setActiveTab] = useState("general");
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [saveError, setSaveError] = useState("");
  const draft = useDraft("cafes", initialDraftId);
  ...
  async function handleSave() {
    ...
    startTransition(async () => {
      try {
        await saveCafe(id ?? null, buildPayload(), draft.draftId);
      } catch (e: unknown) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => router.push("/admin/cafes")}
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FillDataButton category="cafes" name={name} address={address} googlePlaceId={googlePlaceId} instagram={instagram} website={website} onResult={applyFill} disabled={isPending} />
      ...
      {activeTab === "general" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Location Detail">
            <input style={inputStyle} value={locationDetail} onChange={(e) => setLocationDetail(e.target.value)} placeholder="e.g. Emerald Bintaro" />
          </Field>
          ...
          <Field label="Jam Buka">
            <input style={inputStyle} value={hours} onChange={(e) => setHours(e.target.value)} placeholder="e.g. Senin–Minggu 07:00–21:00" />
          </Field>

          <Field label="About (ID)">
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
              <ImproveButton text={about} name={name} category="cafe" onResult={setAbout} />
              <TranslateButton
      ...
      {activeTab === "cafe" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Fasilitas (ID)">
            <textarea style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} value={facilities} onChange={(e) => setFacilities(e.target.value)} placeholder="e.g. Indoor area, Outdoor area, Playground, Kid-friendly" />
          </Field>
        ...
        <div style={{ display: "grid", gap: 28 }}>
          <div>
            ...
            <ImageUpload
              ...
              bucket="cafes-logo"
              path={`cf-${id ?? "new"}/logo_${Date.now()}.jpg`}
