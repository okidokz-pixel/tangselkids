...
export function DaycareForm({ initial, id, initialDraftId }: { initial?: Record<string, any>; id?: string; initialDraftId?: string }) {
  ...
  // Daycare info
  const [ageGroups, setAgeGroups] = useState<string[]>(initial?.age_groups ?? []);
  const [carerChildRatio, setCarerChildRatio] = useState(initial?.carer_child_ratio ?? "");
  const [method, setMethod] = useState(initial?.method ?? "");
  const [hasCctv, setHasCctv] = useState<boolean | "">(
    initial?.has_cctv === true ? true : initial?.has_cctv === false ? false : ""
  );
  const [hasAccreditation, setHasAccreditation] = useState<boolean | "">(
    initial?.has_accreditation === true ? true : initial?.has_accreditation === false ? false : ""
  );
  ...
  function buildPayload() {
    ...
    return {
      ...
      carer_child_ratio: carerChildRatio || null,
      method: method || null,
      has_cctv: hasCctv === "" ? null : hasCctv,
      has_accreditation: hasAccreditation === "" ? null : hasAccreditation,
  ...
  const TABS = ["general", "daycare", "fees", "media"];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => router.push("/admin/daycares")}
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>
            {id ? name || "Edit Daycare" : "Tambah Daycare"}
          </h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FillDataButton category="daycares" name={name} address={address} googlePlaceId={googlePlaceId} instagram={instagram} website={website} onResult={applyFill} disabled={isPending} />
      ...
      {activeTab === "general" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Jam Buka">
            <input style={inputStyle} value={hours} onChange={(e) => setHours(e.target.value)} placeholder="e.g. Senin–Jumat 07:00–17:00" />
          </Field>
      ...
      {activeTab === "daycare" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {AGE_GROUP_OPTIONS.map((g) => (
                <label key={g} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: 13 }}>
                  <input type="checkbox" checked={ageGroups.includes(g)} onChange={() => toggleAgeGroup(g)} />
                  {g}
                </label>
              ))}
            </div>
          ...
          <Field label="Rasio Pengasuh:Anak">
            <input style={inputStyle} value={carerChildRatio} onChange={(e) => setCarerChildRatio(e.target.value)} placeholder="e.g. 1:3, 1:5" />
          </Field>

          <Field label="Metode / Kurikulum">
          ...
          <Field label="CCTV & Akses">
            <select style={inputStyle} value={hasCctv === "" ? "" : hasCctv ? "true" : "false"}
              onChange={(e) => setHasCctv(e.target.value === "" ? "" : e.target.value === "true")}>
          ...
          <Field label="Akreditasi">
            <select style={inputStyle} value={hasAccreditation === "" ? "" : hasAccreditation ? "true" : "false"}
              onChange={(e) => setHasAccreditation(e.target.value === "" ? "" : e.target.value === "true")}>
              <option value="">— Not set —</option>
              <option value="true">Ada</option>
              <option value="false">Tidak Ada</option>
            </select>
      ...
      {activeTab === "fees" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          <SectionDivider label="Biaya Bulanan" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ display: "grid", gap: 28 }}>
          <div>
            ...
            <ImageUpload
              ...
              bucket="daycares-logo"
              path={`dc-${id ?? "new"}/logo_${Date.now()}.jpg`}
          ...
          <div>
            ...
            <PhotoGrid
              ...
              bucket="daycares-photos"
              entityId={id ?? "new"} maxPhotos={10}
            />
          </div>

          <div>
            <SectionDivider label="Detail Image (Premium-gated)" />
            <ImageUpload
              value={feeImageUrl} onChange={setFeeImageUrl}
              bucket="daycares-detail"
              path={`dc-${id ?? "new"}/detail_${Date.now()}.jpg`}
