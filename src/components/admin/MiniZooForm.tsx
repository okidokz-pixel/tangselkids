...
import { saveMiniZoo, deleteMiniZoo } from "@/app/admin/actions";
...
export function MiniZooForm({ initial, id, initialDraftId }: { initial?: Record<string, any>; id?: string; initialDraftId?: string }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeTab, setActiveTab] = useState("general");
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [saveError, setSaveError] = useState("");
  const draft = useDraft("mini-zoo", initialDraftId);
  ...
  async function handleSave() {
    ...
    startTransition(async () => {
      try {
        await saveMiniZoo(id ?? null, buildPayload(), draft.draftId);
      } catch (e: unknown) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => router.push("/admin/mini-zoo")}
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>
            {id ? name || "Edit Animal Encounter" : "Tambah Animal Encounter"}
          </h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FillDataButton category="mini-zoo" name={name} address={address} googlePlaceId={googlePlaceId} instagram={instagram} website={website} onResult={applyFill} disabled={isPending} />
      ...
      {activeTab === "general" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Location Detail">
            <input style={inputStyle} value={locationDetail} onChange={(e) => setLocationDetail(e.target.value)} placeholder="e.g. Pondok Ranji" />
          </Field>
          ...
          <Field label="Jam Buka">
            <input style={inputStyle} value={hours} onChange={(e) => setHours(e.target.value)} placeholder="e.g. Senin–Minggu 09:00–18:00" />
          </Field>

          <Field label="About (ID)">
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
              <ImproveButton text={about} name={name} category="mini-zoo" onResult={setAbout} />
              <TranslateButton
        ...
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Google Rating">
              <input style={inputStyle} type="number" step="0.1" min="1" max="5" value={googleRating} onChange={(e) => setGoogleRating(e.target.value)} placeholder="e.g. 4.5" />
            </Field>
          ...
          <Field label="Fasilitas (ID)">
            <textarea style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} value={facilities} onChange={(e) => setFacilities(e.target.value)} placeholder="e.g. Area bermain, Feeding session, Kandang interaktif" />
          </Field>
        ...
        <div style={{ display: "grid", gap: 28 }}>
          <div>
            ...
            <ImageUpload
              ...
              bucket="zoo-logo"
              path={`mz-${id ?? "new"}/logo_${Date.now()}.jpg`}
          ...
          <div>
            ...
            <ImageUpload
              ...
              bucket="zoo-detail"
              path={`mz-${id ?? "new"}/detail_${Date.now()}.jpg`}
