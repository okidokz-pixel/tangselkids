...
import { saveClinic, deleteClinic } from "@/app/admin/actions";
...
export function ClinicForm({ initial, id, initialDraftId }: { initial?: Record<string, any>; id?: string; initialDraftId?: string }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeTab, setActiveTab] = useState("general");
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [saveError, setSaveError] = useState("");
  const draft = useDraft("clinics", initialDraftId);
  ...
  async function handleSave() {
    ...
    startTransition(async () => {
      try {
        await saveClinic(id ?? null, buildPayload(), draft.draftId);
      } catch (e: unknown) {
  ...
  const TABS = ["general", "clinic", "fees", "media"];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => router.push("/admin/clinics")}
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>
            {id ? name || "Edit Klinik" : "Tambah Klinik"}
          </h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FillDataButton category="clinics" name={name} address={address} googlePlaceId={googlePlaceId} instagram={instagram} website={website} onResult={applyFill} disabled={isPending} />
      ...
      {activeTab === "general" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Jam Buka">
            <input style={inputStyle} value={hours} onChange={(e) => setHours(e.target.value)} placeholder="e.g. Senin–Sabtu 08:00–17:00" />
          </Field>

          <Field label="About (ID)">
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
              <ImproveButton text={about} name={name} category="clinic" onResult={setAbout} />
              <TranslateButton
        ...
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Google Rating">
              <input style={inputStyle} type="number" step="0.1" min="1" max="5" value={googleRating} onChange={(e) => setGoogleRating(e.target.value)} placeholder="e.g. 4.7" />
            </Field>
      ...
      {activeTab === "fees" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          <SectionDivider label="Biaya Terapi" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ display: "grid", gap: 28 }}>
          <div>
            ...
            <ImageUpload
              ...
              bucket="clinics-logo"
              path={`cl-${id ?? "new"}/logo_${Date.now()}.jpg`}
          ...
          <div>
            ...
            <ImageUpload
              ...
              bucket="clinics-detail"
              path={`cl-${id ?? "new"}/detail_${Date.now()}.jpg`}
