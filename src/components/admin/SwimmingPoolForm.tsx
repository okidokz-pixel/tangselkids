...
export function SwimmingPoolForm({ initial, id, initialDraftId }: { initial?: Record<string, any>; id?: string; initialDraftId?: string }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeTab, setActiveTab] = useState("general");
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [saveError, setSaveError] = useState("");
  const draft = useDraft("swimming-pools", initialDraftId);
  ...
  async function handleSave() {
    ...
    startTransition(async () => {
      try {
        await saveSwimmingPool(id ?? null, buildPayload(), draft.draftId);
      } catch (e: unknown) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button type="button" onClick={() => router.push("/admin/swimming-pools")}
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>
            {id ? name || "Edit Kolam Renang" : "Tambah Kolam Renang"}
          </h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FillDataButton category="swimming-pools" name={name} address={address} googlePlaceId={googlePlaceId} instagram={instagram} website={website} onResult={applyFill} disabled={isPending} />
      ...
      {activeTab === "general" && (
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          ...
          <Field label="Location Detail">
            <input style={inputStyle} value={locationDetail} onChange={(e) => setLocationDetail(e.target.value)} placeholder="e.g. Citra Raya" />
          </Field>
          ...
          <Field label="Jam Buka">
            <input style={inputStyle} value={hours} onChange={(e) => setHours(e.target.value)} placeholder="e.g. Selasa–Minggu 08:00–18:00" />
          </Field>
        ...
        <div style={{ display: "grid", gap: 20, maxWidth: 720 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Google Rating">
              <input style={inputStyle} type="number" step="0.1" min="1" max="5" value={googleRating} onChange={(e) => setGoogleRating(e.target.value)} placeholder="e.g. 4.3" />
            </Field>
          ...
          <Field label="Fasilitas (ID)">
            <textarea style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} value={facilities} onChange={(e) => setFacilities(e.target.value)} placeholder="e.g. Kolam anak, Kolam dewasa, Perosotan, Loker" />
          </Field>
        ...
        <div style={{ display: "grid", gap: 28 }}>
          <div>
            ...
            <ImageUpload
              ...
              bucket="pools-logo"
              path={`sp-${id ?? "new"}/logo_${Date.now()}.jpg`}
          ...
          <div>
            ...
            <ImageUpload
              ...
              bucket="pools-detail"
              path={`sp-${id ?? "new"}/detail_${Date.now()}.jpg`}
