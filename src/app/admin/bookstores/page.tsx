...
export default function BookstoresPage() {
  const [items, setItems] = useState<Bookstore[]>([]);
  ...
  useEffect(() => {
    getBookstores().then((data) => { setItems(data); setLoading(false); });
  }, []);

  function handleFeaturedToggle(id: string, current: boolean) {
    setItems((prev) => prev.map((s) => s.id === id ? { ...s, is_featured: !current } : s));
    startTransition(async () => { await toggleBookstoreFeatured(id, !current); });
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>Toko Buku</h1>
          <p style={{ fontSize: 14, color: "#6b7280", marginTop: 4 }}>
            {loading ? "Loading…" : `${filtered.length} of ${items.length} toko`}
          </p>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={bs.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                <td style={{ ...tdStyle, maxWidth: 300 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {bs.photo_1 || bs.logo_url ? (
                      <OptimizedImage src={(bs.logo_url || bs.photo_1) as string} alt="" width={36} height={36} sizes="36px"
                        style={{ width: 36, height: 36, borderRadius: 8, objectFit: "cover", flexShrink: 0, border: "1px solid #e5e7eb" }} />
                    ) : (
                      <div style={{ width: 36, height: 36, borderRadius: 8, background: "#fce7f3", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>📖</div>
                    )}
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>{bs.name}</div>
                      {bs.slug && <div style={{ fontSize: 12, color: "#9ca3af" }}>/{bs.slug}</div>}
                    </div>
                  </div>
                </td>
                <td style={{ ...tdStyle, fontSize: 13, color: "#6b7280" }}>{bs.area || "—"}</td>
                <td style={{ ...tdStyle, textAlign: "center" }}>
                  <button type="button" onClick={() => handleFeaturedToggle(bs.id, bs.is_featured ?? false)}
                    disabled={isPending}
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, opacity: isPending ? 0.5 : 1 }}
                    title={bs.is_featured ? "Remove from featured" : "Mark as featured"}>
                    {bs.is_featured ? "⭐" : "☆"}
                  </button>
                </td>
                <td style={{ ...tdStyle, textAlign: "right" }}>
                  <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", alignItems: "center" }}>
                    ...
                    {bs.slug && (
                      <a href={`/place/${bs.slug}`} target="_blank" rel="noopener noreferrer"
