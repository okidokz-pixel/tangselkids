...
export default function MiniZooPage() {
  const [items, setItems] = useState<MiniZoo[]>([]);
  ...
  useEffect(() => {
    getMiniZoos().then((data) => { setItems(data); setLoading(false); });
  }, []);

  function handleFeaturedToggle(id: string, current: boolean) {
    setItems((prev) => prev.map((s) => s.id === id ? { ...s, is_featured: !current } : s));
    startTransition(async () => { await toggleMiniZooFeatured(id, !current); });
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>Animal Encounter</h1>
          <p style={{ fontSize: 14, color: "#6b7280", marginTop: 4 }}>
            {loading ? "Loading…" : `${filtered.length} of ${items.length} tempat`}
          </p>
        </div>
        <Link href="/admin/mini-zoo/new"
          style={{ padding: "10px 20px", borderRadius: 8, background: "#0e1d4f", color: "#fff", fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
          + Add Animal Encounter
        </Link>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={mz.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                <td style={{ ...tdStyle, maxWidth: 300 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>{mz.name}</div>
                      {mz.slug && <div style={{ fontSize: 12, color: "#9ca3af" }}>/{mz.slug}</div>}
                    </div>
                  </div>
                </td>
                <td style={{ ...tdStyle, fontSize: 13, color: "#6b7280" }}>{mz.area || "—"}</td>
                <td style={{ ...tdStyle, textAlign: "center" }}>
                  <button type="button" onClick={() => handleFeaturedToggle(mz.id, mz.is_featured ?? false)}
                    disabled={isPending}
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, opacity: isPending ? 0.5 : 1 }}
                    title={mz.is_featured ? "Remove from featured" : "Mark as featured"}>
                    {mz.is_featured ? "⭐" : "☆"}
                  </button>
                </td>
                <td style={{ ...tdStyle, textAlign: "right" }}>
                  <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", alignItems: "center" }}>
                    <Link href={`/admin/mini-zoo/new?duplicate=${mz.id}`}
                      title="Duplicate"
                    ...
                    <Link href={`/admin/mini-zoo/${mz.id}`}
                      style={{ padding: "5px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, background: "#dcfce7", color: "#166534", textDecoration: "none" }}>
                      Edit
                    </Link>
                    {mz.slug && (
                      <a href={`/place/${mz.slug}`} target="_blank" rel="noopener noreferrer"
