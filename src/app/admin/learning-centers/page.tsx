...
import { getLearningCenters, toggleLearningCenterFeatured } from "../actions";

type LC = Awaited<ReturnType<typeof getLearningCenters>>[number];
...
export default function LearningCentersPage() {
  ...
  useEffect(() => {
    getLearningCenters().then((data) => { setItems(data); setLoading(false); });
  }, []);

  function handleFeaturedToggle(id: string, current: boolean) {
    setItems((prev) => prev.map((s) => s.id === id ? { ...s, is_featured: !current } : s));
    startTransition(async () => { await toggleLearningCenterFeatured(id, !current); });
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>Tempat Kursus</h1>
          <p style={{ fontSize: 14, color: "#6b7280", marginTop: 4 }}>
            {loading ? "Loading…" : `${filtered.length} of ${items.length} tempat kursus`}
          </p>
        </div>
        <Link href="/admin/learning-centers/new"
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={lc.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                <td style={{ ...tdStyle, maxWidth: 280 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {lc.photo_1 || lc.logo_url ? (
                      <OptimizedImage src={(lc.logo_url || lc.photo_1) as string} alt="" width={36} height={36} sizes="36px"
                    ...
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>{lc.name}</div>
                      {lc.slug && <div style={{ fontSize: 12, color: "#9ca3af" }}>/{lc.slug}</div>}
                    </div>
                  </div>
                </td>
                <td style={{ ...tdStyle, fontSize: 12, color: "#6b7280", maxWidth: 180 }}>
                  {(lc.course_types as string[] | null)?.join(", ") || <span style={{ color: "#d1d5db" }}>—</span>}
                </td>
                <td style={{ ...tdStyle, fontSize: 13, color: "#6b7280" }}>{lc.area || "—"}</td>
                <td style={{ ...tdStyle, textAlign: "center" }}>
                  <button type="button" onClick={() => handleFeaturedToggle(lc.id, lc.is_featured ?? false)}
                    disabled={isPending}
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, opacity: isPending ? 0.5 : 1 }}
                    title={lc.is_featured ? "Remove from featured" : "Mark as featured"}>
                    {lc.is_featured ? "⭐" : "☆"}
                  </button>
                </td>
                <td style={{ ...tdStyle, textAlign: "right" }}>
                  <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", alignItems: "center" }}>
                    <Link href={`/admin/learning-centers/new?duplicate=${lc.id}`}
                      title="Duplicate"
