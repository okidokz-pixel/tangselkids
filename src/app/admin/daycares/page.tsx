...
export default function DaycaresPage() {
  ...
  useEffect(() => {
    getDaycares().then((data) => { setItems(data); setLoading(false); });
  }, []);

  function handleFeaturedToggle(id: string, current: boolean) {
    setItems((prev) => prev.map((s) => s.id === id ? { ...s, is_featured: !current } : s));
    startTransition(async () => { await toggleDaycareFeatured(id, !current); });
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#0e1d4f", margin: 0 }}>Daycares</h1>
          <p style={{ fontSize: 14, color: "#6b7280", marginTop: 4 }}>
            {loading ? "Loading…" : `${filtered.length} of ${items.length} daycares`}
          </p>
        </div>
        <Link href="/admin/daycares/new"
              ...
              <tr key={dc.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                <td style={{ ...tdStyle, maxWidth: 300 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>{dc.name}</div>
                      {dc.slug && <div style={{ fontSize: 12, color: "#9ca3af" }}>/{dc.slug}</div>}
                    </div>
                  </div>
                </td>
                <td style={{ ...tdStyle, fontSize: 13, color: "#6b7280" }}>{dc.area || "—"}</td>
                <td style={{ ...tdStyle, textAlign: "center" }}>
                  <button type="button" onClick={() => handleFeaturedToggle(dc.id, dc.is_featured ?? false)}
                    disabled={isPending}
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, opacity: isPending ? 0.5 : 1 }}
                    title={dc.is_featured ? "Remove from featured" : "Mark as featured"}>
                    {dc.is_featured ? "⭐" : "☆"}
                  </button>
                </td>
                <td style={{ ...tdStyle, textAlign: "right" }}>
                  <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", alignItems: "center" }}>
                    ...
                    {dc.slug && (
                      <a href={`/place/${dc.slug}`} target="_blank" rel="noopener noreferrer"
