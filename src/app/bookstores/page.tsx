...
function BookstoresContent() {
  ...
  useEffect(() => {
    fetchPlacesByCategory("bookstore").then(d => {
      setAllPlaces(d);
  ...
  function goCompare() {
    if (typeof window !== "undefined") localStorage.setItem("compareBookIds", JSON.stringify(compareIds));
    window.location.href = "/compare?type=book";
  }
  ...
  function applyFilters(list: Place[]) {
    return list
      ...
      .filter(b => placeMatchesFeatures(b, feats))
      .filter(b => priceMax == null || b.priceMin <= priceMax);
  }
  ...
  const filtered = sortList(applyFilters(allPlaces).filter(b => b.id !== featuredSpot?.id));
            ...
            return (
              <div key={b.id}>
                {compareMode ? (
                  ...
                  >
                    <PlaceCard
                      place={b} selected={isSelected}
                      distanceKm={
                        locationStatus === "granted" && userLat && userLng && b.lat && b.lng
                          ? haversineKm(userLat, userLng, b.lat, b.lng) : null
                      }
                    />
                  </ActionButton>
                ) : (
                  <Link href={`/place/${b.slug ?? b.id}`} style={{ textDecoration: "none", display: "block" }}>
                    <PlaceCard
                      place={b}
                      distanceKm={
                        locationStatus === "granted" && userLat && userLng && b.lat && b.lng
