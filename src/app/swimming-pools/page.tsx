...
function SwimmingPoolsContent() {
  const { t } = useLang();
  const { userLat, userLng, locationStatus, requestLocation } = useLocation();

  const PRICE_OPTIONS = [
    { value: "all",     label: t.filterAll },
    { value: "lt25",    label: "< Rp 25 rb" },
    { value: "25to45",  label: "Rp 25–45 rb" },
    { value: "45to70",  label: "Rp 45–70 rb" },
  ...
  useEffect(() => {
    fetchPlacesByCategory("swimming-pool").then(d => {
      setAllPlaces(d);
  ...
  function goCompare() {
    if (typeof window !== "undefined") localStorage.setItem("comparePoolIds", JSON.stringify(compareIds));
    window.location.href = "/compare?type=pool";
  }

  function matchesBucket(p: { priceMin: number }): boolean {
    if (priceBucket === "lt25")   return p.priceMin < 25_000;
    if (priceBucket === "25to45") return p.priceMin >= 25_000 && p.priceMin <= 45_000;
    if (priceBucket === "45to70") return p.priceMin > 45_000 && p.priceMin <= 70_000;
  ...
  }, [allFeatured, area, priceBucket, sortSeed]);
    ...
    return (
      <div style={{ maxWidth: 448, margin: "0 auto", minHeight: "100vh", background: "#f6f1e8" }}>
        ...
        <div style={{ padding: "16px 16px 130px", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "10px 14px", border: "1px solid #ede8df" }}>
            <p style={{ margin: "0 0 8px", fontSize: 10, fontWeight: 700, letterSpacing: 1.2,
              color: "#94a3b8", textTransform: "uppercase" }}>{t.filterHargaTiket}</p>
