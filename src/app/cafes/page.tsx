...
function CafesContent() {
  ...
  useEffect(() => {
    fetchPlacesByCategory("cafe").then(d => {
      setAllPlaces(d);
  ...
  function goCompare() {
    if (typeof window !== "undefined") localStorage.setItem("compareCafeIds", JSON.stringify(compareIds));
    window.location.href = "/compare?type=cafe";
  }
  ...
  }, [allFeatured, area, budget, sortSeed]);
    ...
    return (
      <div style={{ maxWidth: 448, margin: "0 auto", minHeight: "100vh", background: "#f6f1e8" }}>
        ...
        <div style={{ padding: "16px 16px 130px", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "10px 14px", border: "1px solid #ede8df" }}>
            <p style={{ margin: "0 0 8px", fontSize: 10, fontWeight: 700, letterSpacing: 1.2,
              color: "#94a3b8", textTransform: "uppercase" }}>{t.filterBudget}</p>
