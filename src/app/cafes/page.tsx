...
const BUDGET_OPTIONS = ["Murah Sekali","Murah","Normal","Agak Mahal","Mahal"] as const;

const BUDGET_DROPDOWN_OPTIONS = [
  { value: "all", label: "Semua" },
  ...BUDGET_OPTIONS.map(v => ({ value: v, label: v })),
];
        ...
        <div style={{ padding: "24px 20px 130px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 700, letterSpacing: 1.2,
              color: "#94a3b8", textTransform: "uppercase", flexShrink: 0, width: 96 }}>Budget</p>
            <div style={{ flex: 1 }}>
              <FilterDropdown value={budget} onChange={setBudget} options={BUDGET_DROPDOWN_OPTIONS} />
            </div>
          </div>
        ...
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 10,
          ...
          <div style={{ maxWidth: 448, margin: "0 auto" }}>
            <button
              ...
              {tier === "guest" ? "Daftar untuk Lihat Semua" : t.cafeShowResults(filtered.length)}
            </button>
          </div>
        </div>
      </div>
    ...
    <div style={{ maxWidth: 448, margin: "0 auto", minHeight: "100vh", paddingBottom: 110, background: "#f8fafc" }}>
      ...
      {activeCount > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, margin: "8px 14px 0", alignItems: "center" }}>
          ...
          {budget !== "all" && <FTag label={budget} onRemove={() => setBudget("all")} />}
