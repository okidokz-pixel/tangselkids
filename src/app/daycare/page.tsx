...
import { daycares, placeMatchesAreas } from "@/lib/mockData";
...
const USIA_OPTIONS = [
  ...
  { value: "Bayi (0–1 thn)",    label: "Bayi (0–1 thn)" },
  { value: "Toddler (1–2 thn)", label: "Toddler (1–2 thn)" },
  { value: "Balita (2–4 thn)",  label: "Balita (2–4 thn)" },
  { value: "Usia 4+ thn",       label: "Usia 4+ thn" },
];
...
function DaycareContent() {
  ...
  const activeCount = [area !== "all", usia !== "all", priceBucket !== "all"].filter(Boolean).length;
  function resetFilters() { setArea("all"); setUsia("all"); setPriceBucket("all"); }
    ...
    return (
      <div style={{ maxWidth: 448, margin: "0 auto", minHeight: "100vh", background: "#f8fafc" }}>
        ...
        <div style={{ padding: "24px 20px 130px" }}>
          ...
          {/* Dropdowns */}
          {([
            { label: "Usia",         value: usia,        set: setUsia,        opts: USIA_OPTIONS },
    ...
    <div style={{ maxWidth: 448, margin: "0 auto", minHeight: "100vh", paddingBottom: 110, background: "#f8fafc" }}>
      ...
      {activeCount > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, margin: "8px 14px 0", alignItems: "center" }}>
          ...
          {usia !== "all" && <FTag label={`Usia: ${usia}`} onRemove={() => setUsia("all")} />}
